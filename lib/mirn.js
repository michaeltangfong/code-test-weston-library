...
class mirn {
    ...
    validateMirn(mirn) {
        ...
        if (!new RegExp(/^[0-9]{8}[0-9PpLl]{1}[0-9Cc]{1}[0-9]{0,1}$/g).test(mirn))
            throw new Error('invalid MIRN, accept ^[0-9]{8}[0-9PpLl]{1}[0-9Cc]{1}[0-9]{0,1}$');
        // check for market existence
        if (!markets[mirn[0]])
            throw new Error();
        // check for jurisdiction existence
        if (!jurisdictions[mirn[1]])
            throw new Error(`invalid MIRN, jurisdiction code ${mirn[1]} (in pos 2) not exist`);
        // check for distributor existence
        if (!jurisdictions[mirn[1]].distributors[mirn[2]])
            throw new Error(`invalid MIRN, distributor code ${mirn[2]} (in pos 3) not exist in ${jurisdictions[mirn[1]].name}`);
        ...
        if (!passRange)
            throw new Error(`invalid MIRN not within range`);
    }
    /**
